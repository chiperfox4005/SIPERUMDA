...
<style>
    ...
    .hero-stats { background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); border-radius: 16px; padding: 32px; color: white; margin-bottom: 24px; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3); }
    ...
    .hero-badge { background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border-radius: 12px; padding: 16px 24px; text-align: center; min-width: 180px; }
    ...
    .metric-card { background: white; border-radius: 12px; padding: 24px 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border: 2px solid #e2e8f0; transition: all 0.3s ease; text-decoration: none; color: inherit; height: 100%; display: flex; flex-direction: column; align-items: center; text-align: center; overflow: hidden; }
    ...
    .quick-action-btn { background: white; border: 2px solid #e2e8f0; border-radius: 12px; padding: 20px 16px; text-align: center; text-decoration: none; color: #334155; transition: all 0.2s ease; cursor: pointer; display: flex; flex-direction: column; align-items: center; gap: 12px; }
    .quick-action-btn:hover { border-color: #3b82f6; background: #eff6ff; color: #1e3a8a; transform: translateY(-2px); }
    .quick-action-icon { width: 48px; height: 48px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; transition: all 0.2s; flex-shrink: 0; }
    ...
    .status-aktif { background: #dbeafe; color: #2563eb; }
...
<div class="container-fluid py-4">
        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                <div class="d-flex align-items-center gap-3 mb-3">
                    @php
                        $user = auth()->user();
                        $fotoUrl = $user->foto_profil ? asset('storage/' . $user->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode($user->nama_lengkap ?? $user->name ?? 'User') . '&background=000000&color=fff&size=120&bold=true';
                        $hour = date('H');
                    ...
                    <div>
                        ...
                        <h1 class="fw-bold mb-1">{{ $user->nama_lengkap ?? $user->name ?? 'Pegawai' }}</h1>
                        <div class="small opacity-75"><i class="bi bi-building me-1"></i> {{ $user->bagian->nama_bagian ?? 'Bagian Pegawai' }}</div>
                    </div>
                </div>
            ...
            <div class="col-md-4 text-md-end mt-3 mt-md-0">
                <div class="hero-badge d-inline-block">
                    <span class="label">Total Permohonan Hari Ini</span>
                    <div class="value">{{ $permohonanBaru ?? 0 }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-lg-3">
            <a href="{{ route('peminjaman-ruangan.index') }}" class="metric-card">
                <div class="metric-icon-wrapper" style="background: #fef3c7;"><i class="bi bi-hourglass-split text-warning"></i></div>
                <div class="metric-value">{{ $permohonanSaya ?? 0 }}</div>
                <div class="metric-label">Permohonan Saya</div>
            </a>
        </div>
        <div class="col-6 col-lg-3">
            <a href="{{ route('agenda.index') }}" class="metric-card">
                <div class="metric-icon-wrapper" style="background: #dbeafe;"><i class="bi bi-calendar-event text-primary"></i></div>
                <div class="metric-value">{{ $totalAgendaHariIni ?? 0 }}</div>
                <div class="metric-label">Agenda Hari Ini</div>
            </a>
        </div>
        <div class="col-6 col-lg-3">
            <a href="{{ route('ruangan.index') }}" class="metric-card">
                <div class="metric-icon-wrapper" style="background: #dcfce7;"><i class="bi bi-door-open text-success"></i></div>
                <div class="metric-value">{{ $ruanganDigunakan ?? 0 }}</div>
                <div class="metric-label">Ruangan Digunakan</div>
            </a>
        </div>
        <div class="col-6 col-lg-3">
            <a href="{{ route('pengumuman.index') }}" class="metric-card">
                <div class="metric-icon-wrapper" style="background: #fee2e2;"><i class="bi bi-megaphone text-danger"></i></div>
                <div class="metric-value">{{ $pengumumanTerbaru->count() ?? 0 }}</div>
                <div class="metric-label">Pengumuman Aktif</div>
            </a>
        </div>
    </div>
    ...
    <!-- 3. QUICK ACTIONS -->
    @php
        $isSekretariat = auth()->user()->hasRole('Sekretariat') || auth()->user()->hasRole('IT Administrator');
    @endphp
    ...
    <div class="quick-action-grid mb-4">
        @if(!$isSekretariat)
            <a href="{{ route('peminjaman-ruangan.create') }}" class="quick-action-btn">
                <div class="quick-action-icon" style="background: #dbeafe;"><i class="bi bi-door-open text-primary"></i></div>
                <div class="quick-action-label">Ajukan Peminjaman Ruangan</div>
            </a>
        @else
            <a href="{{ route('peminjaman-ruangan.approval') }}" class="quick-action-btn">
                <div class="quick-action-icon" style="background: #dbeafe;"><i class="bi bi-clipboard-check text-primary"></i></div>
                <div class="quick-action-label">Approve Peminjaman</div>
            </a>
        @endif

        <a href="{{ route('agenda.create') }}" class="quick-action-btn">
            <div class="quick-action-icon" style="background: #dcfce7;"><i class="bi bi-plus-circle text-success"></i></div>
            <div class="quick-action-label">Buat Agenda</div>
        </a>
        
        <a href="{{ route('surat.index') }}" class="quick-action-btn">
            <div class="quick-action-icon" style="background: #fef3c7;"><i class="bi bi-file-earmark-text text-warning"></i></div>
            <div class="quick-action-label">Buat Surat</div>
        </a>

        <a href="{{ route('pengumuman.create') }}" class="quick-action-btn">
            <div class="quick-action-icon" style="background: #fee2e2;"><i class="bi bi-megaphone text-danger"></i></div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            ...
            <div class="compact-table-wrapper mb-4">
                <table class="table mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr><td colspan="2" class="text-center text-muted py-4"><i class="bi bi-calendar-x d-block mb-2" style="font-size: 2rem;"></i>Tidak ada agenda untuk Anda</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                ...
                                <span class="badge bg-light text-dark border" style="font-size: 0.7rem;">{{ $pengumuman->kategori ?? $pengumuman->prioritas ?? 'Umum' }}</span>
                            </td>
                            <td class="text-end"><small class="text-muted">{{ \Carbon\Carbon::parse($pengumuman->tanggal_publish ?? $pengumuman->created_at)->format('d M Y') }}</small></td>
                        </tr>
                        @empty
            ...
            <!-- 3. PERMOHONAN TERBARU (PEMINJAMAN) -->
            <div class="section-title"><i class="bi bi-clock-history text-primary"></i> Permohonan Terbaru</div>
            <div class="compact-table-wrapper mb-4">
                <table class="table mb-0">
                    <thead><tr><th>Ruangan</th><th>Status</th></tr></thead>
                    <tbody>
                        @forelse(($peminjamanSaya ?? collect())->take(3) as $peminjaman)
                        <tr>
                            <td>
                                <div class="fw-semibold">{{ Str::limit($peminjaman->ruangan->nama_ruangan ?? 'Ruangan', 20) }}</div>
                        ...
                        @empty
                        <tr><td colspan="2" class="text-center text-muted py-4"><i class="bi bi-inbox d-block mb-2" style="font-size: 2rem;"></i>Tidak ada permohonan</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <!-- 4. AGENDA MENDATANG -->
            <div class="section-title"><i class="bi bi-calendar-check text-success"></i> Agenda Mendatang</div>
            <div class="compact-table-wrapper">
                <table class="table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div class="fw-semibold">{{ Str::limit($agenda->judul, 25) }}</div>
                                <small class="text-muted">{{ $agenda->jam_mulai ?? '' }} - {{ $agenda->jam_selesai ?? '' }}</small>
                            </td>
                            <td class="text-end">
                                <span class="status-badge status-aktif">{{ \Carbon\Carbon::parse($agenda->tanggal_mulai)->format('d M Y') }}</span>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="2" class="text-center text-muted py-4"><i class="bi bi-calendar-x d-block mb-2" style="font-size: 2rem;"></i>Tidak ada agenda mendatang</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        mainCalendar.addEventSource({
            events: [
                @foreach($kalenderPengumumans ?? [] as $pengumuman)
                {
                    ...
                    start: '{{ \Carbon\Carbon::parse($pengumuman->tanggal_publish ?? $pengumuman->created_at)->format("Y-m-d") }}',
                    end: '{{ $pengumuman->tanggal_berakhir ? \Carbon\Carbon::parse($pengumuman->tanggal_berakhir)->format("Y-m-d") : \Carbon\Carbon::parse($pengumuman->tanggal_publish ?? $pengumuman->created_at)->format("Y-m-d") }}',
                    url: '{{ route("pengumuman.show", $pengumuman->id) }}',
