...
<div class="container-fluid py-4">
            ...
            <div class="col-md-8">
                <div class="d-flex align-items-center gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <h1 class="fw-bold mb-1">{{ $user->nama_lengkap ?? $user->name ?? 'Sekretariat' }}</h1>
                        <div class="small opacity-75">
                            <i class="bi bi-building me-1"></i> {{ $user->bagian->nama_bagian ?? 'Bagian Sekretariat' }}
                        </div>
                    </div>
                </div>
    ...
    <div class="quick-action-grid mb-4">
        ...
        <a href="{{ route('surat.index') }}" class="quick-action-btn">
            ...
            <div class="quick-action-label">Persetujuan Surat</div>
        </a>
    </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="compact-table-wrapper">
                <table class="table mb-0">
                    <thead><tr><th>Pemohon</th><th>Status</th></tr></thead>
                    <tbody>
                        @forelse($peminjamanMenungguList ?? [] as $peminjaman)
                        <tr>
                            <td>
                                <div class="fw-semibold">{{ Str::limit($peminjaman->pemohon->nama_lengkap ?? $peminjaman->pemohon->name ?? 'User', 20) }}</div>
                                <small class="text-muted">{{ $peminjaman->ruangan->nama_ruangan ?? '-' }}</small>
                            </td>
                            <td><span class="status-badge status-menunggu"><i class="bi bi-hourglass"></i> Menunggu</span></td>
                        </tr>
                        @empty
                ...
                <table class="table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end"><span class="status-badge status-aktif"><i class="bi bi-check"></i> Hari Ini</span></td>
                        </tr>
                        @empty
                        <tr><td colspan="2" class="text-center text-muted py-4"><i class="bi bi-calendar-x d-block mb-2" style="font-size: 2rem;"></i>Tidak ada agenda</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>

            <!-- 3. Pengumuman Terbaru (BARU DITAMBAHKAN) -->
            <div class="section-title"><i class="bi bi-megaphone text-danger"></i> Pengumuman Terbaru</div>
            <div class="compact-table-wrapper">
                <table class="table mb-0">
                    <thead><tr><th>Judul Pengumuman</th><th>Status</th></tr></thead>
                    <tbody>
                        @forelse($pengumumanTerbaru ?? [] as $pengumuman)
                        <tr>
                            <td>
                                <div class="fw-semibold">{{ Str::limit($pengumuman->judul, 30) }}</div>
                                <small class="text-muted">{{ \Carbon\Carbon::parse($pengumuman->tanggal_publish)->format('d M Y') }}</small>
                            </td>
                            <td class="text-end">
                                @if($pengumuman->status == 'publish')
                                    <span class="status-badge status-aktif"><i class="bi bi-check-circle"></i> Aktif</span>
                                @else
                                    <span class="status-badge" style="background: #f1f5f9; color: #64748b;">{{ ucfirst($pengumuman->status) }}</span>
                                @endif
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="2" class="text-center text-muted py-4"><i class="bi bi-megaphone d-block mb-2" style="font-size: 2rem;"></i>Tidak ada pengumuman</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
