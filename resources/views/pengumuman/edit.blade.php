...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold mb-1" style="color: var(--primary-color);">Edit Pengumuman</h2>
            <p class="text-muted mb-0">Perbarui pengumuman yang sudah dibuat.</p>
        </div>
    ...
    <form action="{{ route('pengumuman.update', $pengumuman) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-card">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror" value="{{ old('judul', $pengumuman->judul) }}" required placeholder="Contoh: Jadwal Libur Nasional">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="jenis" class="form-select @error('jenis') is-invalid @enderror" required>
                        ...
                        @foreach($jenisOptions as $opt)
                            <option value="{{ $opt['id'] }}" {{ old('jenis', $pengumuman->jenis) == $opt['id'] ? 'selected' : '' }}>
                                {{ $opt['nama'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="prioritas" class="form-select @error('prioritas') is-invalid @enderror" required>
                        ...
                        @foreach($prioritasOptions as $opt)
                            <option value="{{ $opt['id'] }}" {{ old('prioritas', $pengumuman->prioritas) == $opt['id'] ? 'selected' : '' }}>
                                {{ $opt['nama'] }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" name="tanggal_mulai" class="form-control @error('tanggal_mulai') is-invalid @enderror" value="{{ old('tanggal_mulai', $pengumuman->tanggal_mulai) }}" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Tanggal Selesai</label>
                    <input type="date" name="tanggal_selesai" class="form-control @error('tanggal_selesai') is-invalid @enderror" value="{{ old('tanggal_selesai', $pengumuman->tanggal_selesai) }}">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="mb-3">
                ...
                <select name="target_audience" id="target_audience" class="form-select @error('target_audience') is-invalid @enderror" required>
                    ...
                    <option value="semua_pegawai" {{ old('target_audience', $pengumuman->target_audience) == 'semua_pegawai' ? 'selected' : '' }}>Semua Pegawai (Seluruh Organisasi)</option>
                    <option value="bagian_tertentu" {{ old('target_audience', $pengumuman->target_audience) == 'bagian_tertentu' ? 'selected' : '' }}>Bagian / Sub Bagian Tertentu (Custom)</option>
                </select>
                @error('target_audience') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <!-- CONTAINER CUSTOM HIERARKIS -->
            <div id="container-bagian" class="{{ old('target_audience', $pengumuman->target_audience) == 'bagian_tertentu' ? '' : 'd-none' }}">
                        ...
                        <div class="card-body py-2">
                            ...
                            <div class="form-check mb-2 pb-2 border-bottom">
                                <input class="form-check-input bagian-checkbox" type="checkbox" 
                                       ...
                                       {{ in_array('bagian_'.$bagian->id, old('target_ids', json_decode($pengumuman->target_ids, true)['bagians'] ?? [])) ? 'checked' : '' }}>
                                ...
                                <div class="ms-3 mt-2">
                                    ...
                                    <div class="form-check mb-1">
                                        <input class="form-check-input sub-checkbox" type="checkbox" 
                                               ...
                                               {{ in_array('sub_'.$sub->id, old('target_ids', json_decode($pengumuman->target_ids, true)['sub_bagians'] ?? [])) ? 'checked' : '' }}>
        ...
        <div class="form-card">
            ...
            <div class="mb-3">
                ...
                <textarea name="isi" rows="6" class="form-control @error('isi') is-invalid @enderror" required placeholder="Tulis isi pengumuman di sini...">{{ old('isi', $pengumuman->isi) }}</textarea>
            ...
            <div class="mb-3">
                ...
                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                    ...
                    @foreach($statusOptions as $opt)
                        ...
                        <option value="{{ $opt['id'] }}" {{ old('status', $pengumuman->status) == $opt['id'] ? 'selected' : '' }}>
                            {{ $opt['nama'] }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="d-flex justify-content-end gap-2">
            ...
            <button type="submit" class="btn btn-primary"><i class="bi bi-save me-2"></i>Perbarui Pengumuman</button>
        </div>
    </form>
</div>
